...
void UPlateMovingComponent::BeginPlay()
{
	...
	for (int32 i = 0; i != StaticMeshComps.Num(); ++i)
	{
		...
		if (StaticMeshComps[i]->GetName() == TEXT("PressurePlateMesh"))
		{
...
void UPlateMovingComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (MoveDown)
	{
		if (trans.GetLocation().Z - MoveStep > OriginLocation.Z - MoveDistance)
		{
			...
			auto Dist = FVector::DistSquared(CurrLoc, TargetLocation);
			if (Dist > 0.0f) 
			{
				// need to move
				FMath::VInterpConstantTo(GetOwner()->GetActorLocation(), TargetLocation, DeltaTime, 10.0f);

				Dist = FVector::DistSquared(CurrLoc, TargetLocation);
				if (Dist <= 0.0f)
				{
					// reached end
				}
	...
	if (trans.GetLocation().Z - MoveStep <= OriginLocation.Z - MoveDistance) {
		if (ActivateSomething == false)
		{
			...
			if (TriggeredActor != nullptr)
			{
				...
				if (comp != nullptr)
				{
					ARYUPressurePlate* MyOwner = Cast<ARYUPressurePlate>(this->GetOwner());
					//UE_LOG(LogTemp, Warning, TEXT("Symbol Box: %d "), (int)comp->Symbol);
					//UE_LOG(LogTemp, Warning, TEXT("Symbol Plate: %d "), (int)MyOwner->Symbol);
					if ((int32)comp->Symbol == static_cast<int32>(MyOwner->Symbol))
					{
						auto* world = GetWorld();
						auto* gm = Cast<ARYUGameMode>(world->GetAuthGameMode());
